...
### 6.1 Cloud Run容器运行时深度原理

**Cloud Run是怎么实现容器实例的快速启动和扩缩容的？**
...
Cloud Run基于Knative构建，提供Serverless容器运行体验
...
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  控制平面 (Control Plane)                               │   │
│  │  ├── API Server: 处理CRD资源 (Service, Configuration)│   │
│  │  ├── Activator: 激活冷实例                            │   │
...
│  │  └── 后续启动: 毫秒级（Layer缓存）                    │   │
│  │                                                          │   │
│  │  优化: 使用Distroless镜像，减小体积                    │   │
│  └─────────────────────────────────────────────────────────┘   │
│                              │                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  步骤2: 容器创建 (Container Create)                   │   │
│  │  ├── 分配网络命名空间                                 │   │
│  │  ├── 挂载文件系统                                     │   │
│  │  └── 启动用户进程                                     │   │
│  │  └── 通常: 100-500ms                                  │   │
...
│                              │                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  步骤4: 健康检查通过 (Health Check)                    │   │
│  │  ├── 启动探针 (Startup Probe)                         │   │
│  │  └── 标记为就绪                                       │   │
│  └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘

Knative Autoscaler工作原理：

┌─────────────────────────────────────────────────────────────────┐
│  KPA (Knative Pod Autoscaler) vs HPA对比：                     │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
...
│                                                                  │
│  KPA公式:                                                       │
│  desired_replicas = ceil(current_concurrency / target)         │
│                                                                  │
│  例: target=100, 当前并发=500 → 需要5个实例                   │
└─────────────────────────────────────────────────────────────────┘
...
│                                                                  │
│  1. 请求到达Envoy Sidecar                                      │
│  │   └── 监听: 8080 (用户端口), 15020 (admin端口)           │
│  │                                                               │
│  2. Envoy转发到应用容器                                        │
│  │   └── localhost:8081 (应用实际端口)                        │
│  │                                                               │
│  3. 应用处理请求并返回                                         │
│  │                                                               │
│  4. Envoy记录延迟等指标                                        │
...
┌─────────────────────────────────────────────────────────────────┐
│  Google Compute Engine启动脚本工作原理：                        │
│                                                                  │
│  1. 实例元数据服务 (Metadata Server)                           │
│     ├── IP: 169.254.169.254                                    │
│     ├── 位置: http://metadata.google.internal/computeMetadata/ │
│     └── 认证: OAuth2 Bearer Token                              │
│                                                                  │
│  2. 启动脚本来源：                                             │
│     ├── 元数据键: startup-script (shell脚本)                  │
│     ├── 元数据键: startup-script-win (PowerShell)             │
│     └── 元数据键: startup-script-url (从URL下载)              │
│                                                                  │
│  3. 执行流程：                                                 │
│     ├── google-startup-scripts.service (systemd)              │
│     └── /usr/share/google/startup_scripts/                     │
└─────────────────────────────────────────────────────────────────┘
...
│  │  可用工具（已预装）：                                   │   │
│  │                                                          │   │
│  │  curl, wget, git, docker, gcloud, gsutil               │   │
│  │  pip, npm, node, python, java                          │   │
│  │  systemctl, apt-get, yum                               │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  常用模式：                                             │   │
│  │                                                          │   │
│  │  # 下载文件                                             │   │
│  │  curl -o /tmp/file.sh http://example.com/file.sh      │   │
│  │                                                          │   │
│  │  # 执行系统命令                                         │   │
