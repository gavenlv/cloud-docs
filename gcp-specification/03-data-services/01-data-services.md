...
### 6.1 BigQuery存储格式Capacitor深度原理

**BigQuery是怎么实现高效压缩和快速扫描的？**
...
BigQuery使用Capacitor作为其专有的列式存储格式

┌─────────────────────────────────────────────────────────────────┐
│  为什么列式存储适合分析查询：                                      │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  行式存储 (Row-based) vs 列式存储 (Column-based)        │   │
│  │                                                          │   │
│  │  原始数据:                                              │   │
...
│  │                                                          │   │
│  │  行式存储 (一行连续存放):                               │   │
│  │  [1][Alice][25][NYC][85][2][Bob][30][LA][90]...       │   │
│  │                                                          │   │
│  │  列式存储 (一列连续存放):                               │   │
│  │  [1][2][3] [Alice][Bob][Carol] [25][30][28]...         │   │
│  │                                                          │   │
│  │  分析查询通常只选择少数列:                              │   │
│  │  SELECT avg(score) FROM table WHERE city='NYC'        │   │
│  │  ───────────────────────────────────────────────────── │   │
│  │  只需要读取score和city列                                │   │
│  │  列式存储: 只读2列数据 = 压缩率高，扫描快！            │   │
│  └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
...
│                                                                  │
│  1. 列式编码                                                   │
│     ├── RLE (Run-Length Encoding): 重复值压缩                   │
│  │   例: [NYC][NYC][NYC][NYC][LA][LA] → [NYC,4][LA,2]      │   │
│     ├── Dictionary Encoding: 小基数字符串编码                   │
│  │   例: ["NYC","LA","NYC","NYC"] → Dictionary:{NYC,LA}    │   │
│  │       + [0,1,0,0] (索引)                                 │   │
│     └── Delta Encoding: 数字序列编码                          │   │
│         例: [100][101][102][103] → [100][+1][+1][+1]         │   │
│                                                                  │
│  2. 即时解码 (Just-In-Time Decoding)                           │
│     ├── 数据在磁盘上保持编码状态                               │   │
│     ├── 读取时在CPU指令级别解码                               │   │
│     └── 利用SIMD指令并行处理                                  │   │
│                                                                  │
│  3. 数据分块 (Column Segments)                                 │
│     ├── 每列分成8MB左右的段                                    │   │
│     ├── 每段包含统计信息(min/max/count)                       │   │
│     └── 跳过不相关的段（物化查询规划）                         │   │
└─────────────────────────────────────────────────────────────────┘
...
│  │  数据类型      │  原始大小 │  压缩后 │  压缩率         │   │
│  ├─────────────────────────────────────────────────────────┤   │
│  │  重复字符串    │   100MB   │  ~5MB   │   20:1         │   │
│  │  时间戳        │   100MB   │  ~20MB  │   5:1          │   │
│  │  数字          │   100MB   │  ~30MB  │   3:1          │   │
│  │  UUID          │   100MB   │  ~80MB  │   1.25:1       │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  结果：大多数表在BigQuery中压缩到原来的10-30%                   │
│  这意味着：同样的存储成本，可以存10倍的数据！                   │
└─────────────────────────────────────────────────────────────────┘
```
...
│  }                                                             │
│                                                                  │
│  索引: users_by_name_age                                        │
│  ────────────────────────────────────────────────────────────  │
│  Key: [name="Alice", age=25] → Value: docId                  │
│  Key: [name="Bob", age=30] → Value: docId                    │
│                                                                  │
│  B树查找: O(log n) + 顺序读取结果                              │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
