...
│  │                    用户空间 (Ring 3)                    │   │
│  │                                                          │   │
│  │     应用程序: nginx, mysql, python, bash...            │   │
│  │                                                          │   │
│  │     系统库: glibc, libpthread, libcrypto...           │   │
│  │                                                          │   │
│  └─────────────────────────────────────────────────────────┘   │
│                              │   系统调用接口                    │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │                    内核空间 (Ring 0)                    │   │
│  │                                                          │   │
│  │     系统调用: read, write, fork, exec...               │   │
│  │                                                          │   │
│  │     进程调度: schedule(), context_switch()...         │   │
│  │                                                          │   │
│  │     内存管理: page tables, vmalloc()...               │   │
│  │                                                          │   │
│  │     设备驱动: char/block drivers...                   │   │
│  │                                                          │   │
│  │     文件系统: ext4, xfs, btrfs...                      │   │
│  │                                                          │   │
│  │     网络协议栈: TCP/IP, UDP, Socket...                 │   │
│  │                                                          │   │
│  └─────────────────────────────────────────────────────────┘   │
│                              │   硬件                            │
│  ┌─────────────────────────────────────────────────────────┐   │
...
│  │  ├── Wants: 弱依赖（启动失败不阻止）                   │   │
│  │  ├── Before/After: 启动顺序                            │   │
│  │  └── WantedBy/RequiredBy: 反向依赖                     │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  启动流程：                                              │   │
│  │  1. 读取配置 /etc/systemd/system/*.service            │   │
│  │  2. 解析依赖关系，构建依赖图                            │   │
│  │  3. 按拓扑顺序启动（多线程并行）                       │   │
│  │  4. 监控服务状态                                        │   │
│  │  5. 自动重启失败服务                                    │   │
