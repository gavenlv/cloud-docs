...
防火墙规则的评估顺序（第一个匹配的规则决定行为）：

┌─────────────────────────────────────────────────────────────────┐
│                                                                  │
│  1. 实例级徽标（Instance Tags）                                  │
│     └── 最高优先级                                             │
│     └── 如果实例有徽标，查找匹配的target tag规则                 │
│                                                                  │
│  2. 服务账号级别（Service Account Scoping）                      │
│     └── 如果使用服务账号级别规则                                 │
│     └── 根据服务账号匹配                                        │
│                                                                  │
│  3. 网络级默认规则                                               │
...
│                                                                  │
│  Targets（目标）：                                               │
│  - targetTags：应用到哪些实例（通过实例徽标匹配）               │
│  - targetServiceAccounts：应用到哪些服务账号                   │
│  - 无target字段：应用到VPC内所有实例                            │
│                                                                  │
│  Sources/ Destinations（来源/目标）：                            │
│  - sourceTags：源实例徽标（入站规则）                           │
│  - sourceServiceAccounts：源服务账号（入站规则）               │
│  - sourceRanges：源IP范围（CIDR）                               │
│  - destinationRanges：目标IP范围（出站规则）                   │
│                                                                  │
│  Protocols & Ports（协议和端口）：                               │
│  - all 或指定协议（tcp/udp/icmp）                               │
│  - 指定端口或端口范围                                           │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
# 最高优先级规则（拒绝所有SSH）
gcloud compute firewall-rules create block-ssh `
    --network=my-vpc `
    --priority=10 `
    --direction=INGRESS `
    --action=deny `
...
IAM Policy是一个JSON对象，包含以下部分：

{
  "version": "2023-01-01",           # 策略版本（必须）
  "etag": "BwWiCd1RaZs=",           # 用于并发控制
  "bindings": [                      # 绑定数组
    {
      "role": "roles/storage.objectAdmin",  # 角色
      "members": [                           # 成员列表
...
│  serviceAccount: - 服务账号                                      │
│  group:      - Google群组                                        │
│  domain:     - 整个Google Workspace域名                          │
│  special:   - special.mail.google.com（Google内部）           │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
...
┌─────────────────────────────────────────────────────────────────┐
│                                                                  │
│  预定义角色：roles/服务.资源.动作                                │
│                                                                  │
│  例：roles/compute.instanceAdmin                                │
│      ├── compute = 服务                                        │
│      ├── instance = 资源                                       │
│      └── admin = 动作集合                                      │
│                                                                  │
│  自定义角色：organizations/XX/roles/自定义名称                  │
│  或 projects/XX/roles/自定义名称                               │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
3. 评估策略
   ...
   │                                                              │
   │  对于每个binding：                                           │
   │  1. 检查principal是否匹配members                            │
   │  2. 检查role是否包含请求的permission                       │
   │  3. 如果有condition，评估condition表达式                   │
   │  4. 如果全部匹配，返回ALLOW                                 │
   │                                                              │
   │  最终结果：                                                  │
   │  - 任何一个binding允许 = 允许                               │
...
│                                                                  │
│  Project级绑定：                                                │
│  ├── alice@email.com → roles/viewer                           │
│  └── 项目所有资源默认继承viewer                                │
│                                                                  │
│  BigQuery数据集级绑定：                                         │
│  ├── bob@email.com → roles/bigquery.dataEditor               │
│  └── 只影响这个数据集                                          │
│                                                                  │
│  alice的最终权限：                                              │
│  ├── 项目级：viewer（继承）                                     │
│  ├── BigQuery数据集：viewer + bigquery.dataEditor（叠加）     │
│  └── 其他服务：viewer（继承）                                   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
┌─────────────────────────────────────────────────────────────────┐
│  场景: VM-A (10.0.1.5) → VM-B (10.0.2.10)                    │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  1. VM-A发送数据包                                    │   │
│  │     ├── 应用 → Socket → TCP/IP协议栈                  │   │
...
│  │  流量类型        │  路径              │  延迟  │  带宽   │   │
│  ├─────────────────┼─────────────────────┼────────┼─────────┤   │
│  │  同VPC同区域    │  完全内部转发      │  <1ms  │  最大   │   │
│  │  同VPC跨区域    │  跨区域内部网络    │  ~1ms  │  较低   │   │
│  │  出站到公网    │  NAT/Proxy网关     │  ~5ms  │  受限   │   │
│  │  VPC对等        │  Google骨干网      │  ~2ms  │  较高   │   │
│  └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
```

### 6.2 Cloud Armor DDoS防护深度原理

**Cloud Armor是怎么防护DDoS攻击的？**
...
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  1. SQL注入防护                                         │   │
│  │  request.uri contains "union select"                   │   │
│  │  request.uri contains "1=1"                           │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  2. XSS防护                                             │   │
│  │  request.uri contains "<script>"                      │   │
│  │  request.uri contains "javascript:"                    │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  3. 路径遍历防护                                        │   │
