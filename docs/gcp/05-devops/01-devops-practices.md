...
│            '-t', 'gcr.io/PROJECT/IMAGE:$COMMIT_SHA',          │
│            '.']                                                │
│                                                                  │
│  images:                                                       │
│  - 'gcr.io/PROJECT/IMAGE:$COMMIT_SHA'                        │
│  - 'gcr.io/PROJECT/IMAGE:latest'                             │
│                                                                  │
│  关键点：                                                       │
│  1. 使用 --cache-from 引用之前的镜像                         │
│  2. 每次构建后更新latest标签镜像                             │
│  3. 不被使用的缓存会自动被垃圾回收                           │
└─────────────────────────────────────────────────────────────────┘
...
│    args: ['cp', 'gs://BUCKET/m2-cache.tar.gz', '/workspace/'] │
│                                                                  │
│  # 构建前解压缓存                                              │
...
│    args:                                                      │
│    - '-c'                                                     │
│    - 'if [ -f /workspace/m2-cache.tar.gz ]; then            │
│         tar -xzf /workspace/m2-cache.tar.gz -C ~;            │
│         fi; mvn clean package -DskipTests'                   │
│                                                                  │
│  # 构建后保存缓存                                              │
│  - name: gcr.io/cloud-builders/gsutil                        │
│    args: ['cp',                                               │
│            '~/.m2/repository.tar.gz',                         │
│            'gs://BUCKET/m2-cache.tar.gz']                    │
│    env: ['MAVEN_OPTS=-Dmaven.repo.local=/root/.m2/repository']│
└─────────────────────────────────────────────────────────────────┘
```

### 7.2 Artifact Registry漏洞扫描深度原理

**Artifact Registry是怎么检测容器镜像漏洞的？**
...
┌─────────────────────────────────────────────────────────────────┐
│  CVSS评分 (Common Vulnerability Scoring System):              │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
...
│  基于严重程度的部署阻断：                                         │
│                                                                  │
│  gcloud artifacts docker images add-vulnerability-occurrence \ │
│      IMAGE_URI \                                               │
│      --severity=CRITICAL \                                     │
│      --blocking=true                                          │
│                                                                  │
